...
class ProviderAgent {
    static generateBid(invoice, provider, risk) {
        // 1. Dynamic Pricing Curve: Base rate + risk spread + tenor spread
        let rate = provider.baseInterestRatePercent;
        // Risk spread adjustment
        ...
        // Tenor adjustment (>60 days adds small term liquidity premium)
        if (invoice.tenorDays > 60) {
            rate += ((invoice.tenorDays - 60) / 30) * 0.3;
        }
        rate = Math.round(rate * 10) / 10;
        // 2. Advance Rate calculation
        // Conservative bank caps at 70%, NBFC goes up to 90%, Fintech up to 95%
        let advanceRate = provider.maxAdvanceRate;
        // If high risk, prune advance slightly
        ...
        // Total financing cost = Interest for tenor + Origination fee
        ...
        const offer = {
            ...
            utilityScore: 0, // Populated by Multi-Dimensional Clearing Engine
            scoreBreakdown: {
                rateScore: 0,
                advanceScore: 0,
                speedScore: 0,
