import { Invoice, Supplier, Buyer, VerificationResult, AgentLog } from '../models/types';

export class VerificationEngine {
  public static verifyInvoice(invoice: Invoice, supplier?: Supplier, buyer?: Buyer): { result: VerificationResult; log: AgentLog } {
    const flags: string[] = [];
    ...
    let score = 100;
    if (!gstinActive) score -= 40;
    if (!eWayBillValid) score -= 35;
    if (threeWayMatchScore < 80) score -= (80 - threeWayMatchScore);
    if (!buyerAcknowledged) score -= 15;
    ...
    const log: AgentLog = {
      ...
      message: `Invoice [${invoice.invoiceNumber}] 3-way verification evaluated: ${status} (Score: ${score}/100).`,
      details: result
    };
